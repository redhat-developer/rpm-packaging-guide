...
[NOTE]
====
In the previous versions of Red Hat Enterprise Linux, the ``BUILD`` directory was removed during the package build.
Consequently the ``rpmbuild`` process failed. To fix this, make sure you have removed following line from the ``~/.rpmmacros`` SPEC file:
----
