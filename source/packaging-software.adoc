...
ifdef::community[https://en.wikipedia.org/wiki/Cpio[cpio]] ifdef::rhel[cpio]
...
ifdef::rhel[chroot] environment. This means that the build artifacts are placed
...
ifdef::rhel[shebang] is no longer applicable because the resulting file will not
...
ifdef::community[https://en.wikipedia.org/wiki/Chroot[chroot]]
ifdef::rhel[chroot] base directory with nothing in it and we will have to
...
ifdef::rhel[shebang] line in it when we byte compile that we will need to create
