...
// FIXME DELETE? OR USE CONDITIONALS?
...
to any particular Linux distribution. However, this document targets
// FIXME CONDITIONAL
Fedora, CentOS, and RHEL, so you will notice that:

* We remove the use of ``rm $RPM_BUILD_ROOT``, because it is unnecessary to
perform that task when building on
https://www.redhat.com/en/technologies/linux-platforms[RHEL] or _CentOS_ 7.0 or
newer or on https://getfedora.org/[Fedora] version 18 or newer.
// FIXME CONDITIONAL
* We favor the use of ``%{buildroot}`` notation over
``$RPM_BUILD_ROOT`` when referencing RPM’s Buildroot for consistency with
all other defined or provided macros throughout the SPEC file.
...
bello version ``0.2`` is released. The __disttag__ macro is covered in LINK.
// FIXME LINK TO PART ABOUT MACROS
+
...
// FIXME LINK FOR RHEL GUIDELINES?
...
// FIXME WHAT DOES THIS MEAN?? FIND OUT & REWRITE
+
...
==== pello
// FIXME REWRITE PELLO EXAMPLE SIMILAR TO BELLO ABOVE
...
//FIXME FIX STRUCTURE, HERE SHOULD BE A BRIDGEHEAD OR SOMETHING
...
This is all there is to building a Source RPM.
...
[[rebuild]]
==== Rebuilding from a SRPM

To rebuild ``bello``, ``pello``, and ``cello`` from SRPMs, run:
...
analysis of the RPM. This utility can check Binary RPMs, Source RPMs, and SPEC
