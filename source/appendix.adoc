...
This conditional handles compatibility between RHEL6 and RHEL7. It checks whether the version of RHEL is 6. If it is, then the AS_FUNCTION_DESCRIBE macro, which is not supported by RHEL6, is deleted.
...
This conditional handles compatibility between Fedora and RHEL6. RHEL6 differs from Fedora in the way that the ruby_sitearch macro is not defined by default.
The conditional checks whether the operating system is RHEL6. If it is, ruby_sitearch is defined explicitly. 


===== The %ifarch Conditional

The %ifarch conditional is a special case of RPM conditional, which uses %ifarch instead of %if. The %ifarch conditional is used to begin a block of the SPEC file that is architecture-specific. It is followed by one or more architecture specifiers, each separated by commas or whitespace. 

  %ifarch i386 sparc
  ...
  %endif
  
All **the contents** of the SPEC file ** between %ifarch and %endif** are processed only on Intel x86 or Sun SPARC-based systems. 


===== The %ifnarch Conditional

The %ifnarch conditional is a special case of RPM conditional, which uses %ifnarch instead of %if. The %ifnarch conditional has a reverse logic than %ifarch conditional.
...
The %ifos conditional is a special case of RPM conditional, which uses %ifos instead of %if. The %ifos conditional is used to control processing based on the operating system of the build. It can be followed by one or more operating system names. 
...
The following conditional handles compatibility between RHEL6 and RHEL7. It checks whether the version of RHEL is 6. If it is, then the AS_FUNCTION_DESCRIBE macro, which is not supported by RHEL6, is deleted.
