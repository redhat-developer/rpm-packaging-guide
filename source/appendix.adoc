...
In order to use https://fedoraproject.org/wiki/Mock[Mock] on a RHEL or CentOS system, you will need to enable
...
The https://github.com/dgoodwin/tito[tito] package is available in https://getfedora.org/[Fedora] as well as in the https://fedoraproject.org/wiki/EPEL[EPEL]
...
=== RPM Distribution Macros#rpm-distribution-macros[¶]
...
== Advanced SPEC File Topics#advanced-spec-file-topics[¶]
...
=== Triggers and Scriptlets#triggers-and-scriptlets[¶]
...
system administrator can run a command similar to 
[source,java]
----
systemctl start
foo.service
----
 after the fictional RPM ``foo`` (which provides some service
...
available languages can be found in the <cite>RPM Official Documentation</cite>.
...
*   <cite>Maximum RPM</cite> (Some material is dated, but this is still a great resource for
advanced topics.)
