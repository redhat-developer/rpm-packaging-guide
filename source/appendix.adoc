...
ifdef::rhel[]
There are also macros that are provided by your https://en.wikipedia.org/wiki/Linux[Linux] Distribution, we will cover some of those provided by https://www.redhat.com/en/technologies/linux-platforms[RHEL] in this section as well as provide information on how to inspect your system to learn about others that we don’t cover or for discovering them on other RPM-based https://en.wikipedia.org/wiki/Linux[Linux]
Distributions.
endif::rhel[]
...
This conditional handles compatibility between RHEL6 and other operating systems in terms of support of the AS_FUNCTION_DESCRIBE macro. The conditional checks whether the version of RHEL is 6. If it is, then AS_FUNCTION_DESCRIBE, which is not supported by RHEL6, is deleted.
...
This conditional handles compatibility between the recent version of Fedora and RHEL6 in terms of support of the ruby_sitearch macro. The recent version of Fedora defines ruby_sitearch by default, but RHEL6 does not support this macro. The conditional checks whether the operating system is RHEL6. If it is, ruby_sitearch is defined explicitly. 
endif::community[]
...
This conditional handles definition of the macros. If ... macros are set, ... macros are defined.
//FIXME which macros?
