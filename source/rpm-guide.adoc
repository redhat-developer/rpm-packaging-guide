...
// FIXME CONDITIONALS - why these conditionals distinguishing between the upsream and downstream version does not work?
...
// * We remove the use of ``rm $RPM_BUILD_ROOT``, because it is unnecessary to perform that task when building on https://www.redhat.com/en/technologies/linux-platforms[RHEL] or _CentOS_ 7.0 or newer or on // https://getfedora.org/[Fedora] version 18 or newer.
// FIXME CONDITIONAL
...
[NOTE]
====
The ``rpmdev-newspec`` utility does not use guidelines or conventions specific to any particular Linux distribution.
...
// FIXME CONDITIONAL - why conditional here?
...
// FIXME WHAT DOES THIS MEAN?? FIND OUT & REWRITE - consult with Max
+
...
// FIXME CONDITIONALS - I do not understand why there shoudl be conditionals and where to put it. 
