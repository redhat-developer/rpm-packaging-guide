...
tied to a proper RPM Build System such as https://pagure.io/koji[Koji].
...
https://rpm-software-management.github.io/rpm/manual/macros.html[RPM Official Documentation], which provides
...
``/var/tmp/rpm-tmp.DhddsG`` use the ``--debug`` option, since ``rpmbuild`` deletes
...
It is also common for RPM Macros to exist for this function. In our previous
