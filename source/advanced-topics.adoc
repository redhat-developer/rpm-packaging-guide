...
Options ``-a`` and ``-b`` expand specific sources. ``-b``, meaning before, expands specific sources prior entering the working directory, while ``-a``, meaning after, expands those sources after entering. Their arguments are source numbers from the spec file preamble.
