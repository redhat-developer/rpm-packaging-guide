...
You can find in some older example use of `%define <name> <body>`. Note that `%define` and `%global` differ in scope. '%define' has local scope while `%global` has global scope. Additionailly the body of a `%define`’d macro is lazily expanded (i.e. when used), but the body of `%global` is expanded at definition time.
