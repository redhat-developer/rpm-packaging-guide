...
Options ``-a`` and ``-b`` expand specific sources. ``-b`` expands specific sources before entering the working directory, while ``-a`` expands those sources after entering. Their arguments are source numbers from the spec file preamble.
