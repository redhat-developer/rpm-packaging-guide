...
Signing a package is a way to secure the package for an end user. Secure transport can be achieved with implementation of the HTTPS protocol, which can be done when the package was downloaded just before installing. However, the packages are often downloaded in advance and stored in local repositories before they are used. The packages are signed to make sure no third party can alter the content of a package.
...
In most cases packages are build without a signature. The signature is added just before release of the package.

In order to add another signature to the package use the --addsign option.
