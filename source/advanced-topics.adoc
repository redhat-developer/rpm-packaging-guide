...
In comparison with the Python Package Index (PyPI), RPM allows to specify dependencies of other RPMs that are not necessary Python packages. RPM allows to use cryptographic signing, content of RPM packages is verified, integrated, and tested with the rest of the system. More can be found in the chapter xref:Why-Package-Software-with-RPM[].
...
There is a chapter xref:what-is-a-spec-file[], which describes spec file in general. Python RPM spec file specifications to spec files shown already in this guide might be shown in this example:
