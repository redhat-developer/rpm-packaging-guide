...
The use of `%define <name> <body>` can be found in older examples. Note that `%define` and `%global` differ in scope. `%define` has local scope while `%global` has global scope. Additionally, the body of a `%define`’d` macro is lazily expanded when used, but the body of `%global` is expanded at definition time.
